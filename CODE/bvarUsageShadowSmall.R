# -----------------------------------------------------------------------------
# SHADOW BANKING (CORE) SECTOR BVAR
# -----------------------------------------------------------------------------

# 1. PREPARE SHADOW CORE SECTOR DATA
shadow_sector_bvar <- shadow_variables_filtered_small %>%
...
all_plots_mp_risk_shadow_core  <- list()
all_plots_facet_shadow_core    <- list()
all_cor_tables_shadow_core     <- list()

for (risk_idx in RISK_INDICES) {

  display_name <- RISK_DISPLAY_NAMES[[risk_idx]]
  cat(sprintf("\n\n========== SHADOW BANKING (CORE) | RUNNING: %s ==========\n", display_name))
  ...
  result <- tryCatch({
    bvar_risk_channel(
      sector_dfs  = list(ShadowCore = shadow_sector_bvar),
      ...
      sector_vars = list(
        ShadowCore = c("Shadow_WholeSaleFunding", "Shadow_LiquidityBuffer",
      ...
      plot         = TRUE,
      title_prefix = sprintf("Shadow Banking (Core) | %s", display_name),
      verbose      = FALSE
    )
  }, error = function(e) {
  ...
  p_facet     <- result$plots[["chol_facet_ShadowCore"]]

  if (!is.null(p_mp_risk)) all_plots_mp_risk_shadow_core[[risk_idx]] <- p_mp_risk
  if (!is.null(p_facet))   all_plots_facet_shadow_core[[risk_idx]]   <- p_facet

  if (!is.null(result$vcov_cholesky)) {
    cor_mat <- round(cov2cor(result$vcov_cholesky), 3)
    all_cor_tables_shadow_core[[risk_idx]] <- cor_mat
    cat(sprintf("  [OK] %s — correlation matrix extracted\n", display_name))
  }
}

# 3. SAVE INDIVIDUAL PDFs
out_dir_shadow_core <- "~/Documents/Projects/Ma_Thesis/CODE/bvar_plots/shadow_core"
dir.create(out_dir_shadow_core, showWarnings = FALSE, recursive = TRUE)

for (risk_idx in RISK_INDICES) {

  p1           <- all_plots_mp_risk_shadow_core[[risk_idx]]
  p2           <- all_plots_facet_shadow_core[[risk_idx]]
  display_name <- RISK_DISPLAY_NAMES[[risk_idx]]
  suffix       <- ifelse(risk_idx %in% FLIP_INDICES, "_flipped", "")
  file_name    <- gsub("[^A-Za-z0-9_]", "_", display_name)

  if (!is.null(p1)) {
    pdf(file.path(out_dir_shadow_core, sprintf("%s%s_shadow_to_risk.pdf", file_name, suffix)),
        width = 10, height = 5)
    print(p1)
    dev.off()
  }
  ...
  if (!is.null(p2)) {
    pdf(file.path(out_dir_shadow_core, sprintf("%s%s_risk_to_shadow_core.pdf", file_name, suffix)),
        width = 12, height = 7)
    print(p2)
    dev.off()
  }
...
cairo_pdf("~/Documents/Projects/Ma_Thesis/CODE/bvar_tables_shadow_core.pdf",
          width = 14, height = 8, onefile = TRUE)

for (risk_idx in names(all_cor_tables_shadow_core)) {
  display_name <- RISK_DISPLAY_NAMES[[risk_idx]]
  cor_mat      <- all_cor_tables_shadow_core[[risk_idx]]
  ...
  grid.newpage()
  grid.text(
    sprintf("Shadow Banking (Core) | Residual Correlation Matrix — %s%s",
...
cat("\n===== SHADOW BANKING (CORE) DONE =====\n")
cat(sprintf("Models estimated: %d / %d\n",
            length(all_cor_tables_shadow_core), length(RISK_INDICES)))
